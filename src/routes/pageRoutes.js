    ...
    upload.single("avatar")(req, res, (err) => {
        if (err) {
            return res.status(400).render("pages/signup", {
                authError: err.message || "Failed to upload avatar."
            });
        }
...
router.post("/profile/avatar", requireAuth, (req, res, next) => {
    upload.single("avatar")(req, res, (err) => {
        ...
        return authController.updateAvatar(req, res, next);
    });
});
...
// responses
router.post("/items/:id/responses", noticeController.createResponseForNotice);
...
// edit notice
router.get("/notices/:id/edit", requireAuth, noticeController.renderEditNotice);
router.post("/notices/:id", requireAuth, upload.array("images", 5), noticeController.updateNotice);
